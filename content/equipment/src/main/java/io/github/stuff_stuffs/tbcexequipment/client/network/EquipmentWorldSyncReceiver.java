...
public final class EquipmentWorldSyncReceiver {
    public static void init() {
        ClientPlayNetworking.registerGlobalReceiver(EquipmentWorldSyncSender.IDENTIFIER, EquipmentWorldSyncReceiver::receive);
    }
    ...
    private static void receive(final MinecraftClient client, final ClientPlayNetworkHandler handler, final PacketByteBuf buf, final PacketSender sender) {
        ...
        ((ClientEquipmetWorldViewHolder) client).tbcex$setMaterialManagerMaps(keyed);
    }
    ...
    private EquipmentWorldSyncReceiver() {
    }
}
