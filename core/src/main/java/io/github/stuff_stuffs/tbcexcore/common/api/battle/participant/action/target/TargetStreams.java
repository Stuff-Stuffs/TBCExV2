...
public final class TargetStreams {
    public static final class Context {
        ...
        public BattleParticipantStateView getState(final BattleParticipantHandle handle) {
            if (!handle.getParent().equals(battle.getHandle())) {
    ...
    public static Stream<BlockPos> getFloorPositions(final Context context, final Stream<BlockPos> positions, final int range) {
        final BlockPos.Mutable mutable = new BlockPos.Mutable();
        final int sq = range * range;
        return positions.flatMap(pos -> Stream.iterate(new FloorFinder(range, pos.getX() - range, pos.getY() - range, pos.getZ() - range), f -> f.curZ < f.z + f.range + 1, floorFinder -> {
        ...
        }).map(f -> new BlockPos(f.curX, f.curY, f.curZ))).distinct();
    }
    ...
    public static Predicate<BattleParticipantHandle> team(final Context context, final boolean same) {
        final BattleParticipantTeam team = context.getSelfState().getTeam();
